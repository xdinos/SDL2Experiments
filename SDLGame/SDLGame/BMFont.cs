...
namespace SDLGame
{
	public class BMFont
	{
		...
		public static BMFont Load(IntPtr renderer, string path, string filename)
		{
		...
		public void DrawText(IntPtr renderer, string text, float x, float y, byte r, byte g, byte b)
		{
			...
			foreach (var c in text)
			{
				if (_glyphs.TryGetValue(c, out var glyph))
				{
					...
					SDL.SDL_RenderCopy(renderer, glyph.Texture.Handle, ref src, ref dst);
