...
namespace Lunatics.Mathematics
{
	...
	public struct Vector3 : IEquatable<Vector3>, IFormattable
	{
		...
		{
			get { return MathUtils.IsOne((X * X) + (Y * Y) + (Z * Z)); }
		}
		...
		{
			return MathUtils.WithinEpsilon(left.X, right.X, epsilon.X) &&
					MathUtils.WithinEpsilon(left.Y, right.Y, epsilon.Y) &&
					MathUtils.WithinEpsilon(left.Z, right.Z, epsilon.Z);
		}
		...
		{
			return MathUtils.NearEqual(other.X, X) && MathUtils.NearEqual(other.Y, Y) && MathUtils.NearEqual(other.Z, Z);
		}
