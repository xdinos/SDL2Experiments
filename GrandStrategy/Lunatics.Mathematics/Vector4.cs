...
namespace Lunatics.Mathematics
{
	...
	public struct Vector4 : IEquatable<Vector4>, IFormattable
	{
		...
		public Vector4(float[] values)
		{
			...
			if (values.Length != 4) throw new ArgumentOutOfRangeException(nameof(values), "There must be four and only four input values for Vector4.");
		...
		{
			get { return MathUtils.IsOne((X * X) + (Y * Y) + (Z * Z) + (W * W)); }
		}
		...
		{
			result.X = MathUtils.Lerp(start.X, end.X, amount);
			result.Y = MathUtils.Lerp(start.Y, end.Y, amount);
			result.Z = MathUtils.Lerp(start.Z, end.Z, amount);
			result.W = MathUtils.Lerp(start.W, end.W, amount);
		}
		...
		{
			return (MathUtils.NearEqual(other.X, X) &&
				MathUtils.NearEqual(other.Y, Y) &&
				MathUtils.NearEqual(other.Z, Z) &&
				MathUtils.NearEqual(other.W, W));
		}
